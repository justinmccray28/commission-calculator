...
function sameOrigin(req) { return req.headers.origin === origin && (req.headers['content-type'] || '').startsWith('application/x-www-form-urlencoded'); }

const html = await readFile(join(directory, 'index.html'), 'utf8');
const commissionData = await readFile(join(directory, 'commission-data.json'), 'utf8');
JSON.parse(commissionData);
const protectedHTML = html.replace("fetch('./commission-data.json'", "fetch('/api/commission-data'").replace('<body>', '<body><div style="display:flex;justify-content:flex-end;padding:10px 24px 0"><form method="POST" action="/auth/logout"><button style="border:1px solid #38748e;border-radius:7px;background:#102a40;color:#e5f8ff;padding:7px 14px;cursor:pointer">Sign out</button></form></div>');
