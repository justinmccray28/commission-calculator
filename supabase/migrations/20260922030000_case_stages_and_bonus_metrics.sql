...
begin
  ...
  select jsonb_build_object(
    ...
    'incoming_requests',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'agent_id',r.agent_id,'agent_name',p.display_name,'contract_level',p.contract_level,'created_at',r.created_at) order by r.created_at) from public.upline_link_requests r join public.agent_profiles p on p.user_id=r.agent_id where r.requested_upline_id=v_user and r.status='pending'),'[]'::jsonb),
    'outgoing_request',(select jsonb_build_object('id',r.id,'upline_name',p.display_name,'status',r.status) from public.upline_link_requests r join public.agent_profiles p on p.user_id=r.requested_upline_id where r.agent_id=v_user and r.status='pending' order by r.created_at desc limit 1),
