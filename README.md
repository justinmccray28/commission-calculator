...
To release a change, edit `index.html` and commit it to `main`. GitHub Pages publishes the new version from the root of that branch. Refresh the webpage to load the update; teammates use the same URL. Publishing changes can take up to 10 minutes. Opening a downloaded HTML file inside ChatGPT may not run its JavaScript, so use the published page.
