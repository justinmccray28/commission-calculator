...
Payouts are calculated in the visitor's browser. A calculation is stored only when a signed-in agent selects **Save calculation** and supplies a client or case name. Saved cases can be viewed and deleted from the **Tracking** tab, which displays combined commission and total allocated agent points. A downline-business case labels its points as downline agent points so override income is not mistaken for personal production.
