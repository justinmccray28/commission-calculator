...
The additive schema is in `supabase/migrations/20260922000000_organization_tree.sql`. It creates `agent_profiles`, `organization_invites`, and `upline_requests` with RLS enabled. The recursive organization query and invitation workflows run through authenticated RPC functions that explicitly verify `auth.uid()`, prevent self-links and circular hierarchies, and expose only descendant metrics. Do not apply this migration to the pilot database until the feature branch is connected to an isolated Supabase development branch or the additive production-schema approach is explicitly approved.
